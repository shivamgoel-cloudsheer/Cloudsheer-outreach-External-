...
// positive-reply rate, and per-mailbox (sender) health. Admins see every
// campaign; clients see every campaign on their own domain.
export async function GET() {
  ...
  if (!access.can.viewAnalytics) return forbidden();

  // null => admin (no restriction); otherwise the ids of the viewer's domain.
  ...
  // Owner labels when more than one owner is in view.
  ...
  const campaignRows = camps.map((c) => {
    ...
    return {
      ...
      name: c.name,
      status: c.status,
      owner: showOwners ? ownerById.get(c.userId) ?? null : null,
